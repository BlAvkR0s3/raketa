...
const int EEPROM_POMIAR_ADDR = 2;  // nowy adres na licznik pomiarów
...
void setup() {
  ...
  if (SD.begin(chipSelect)) {
    ...
    // Odczytaj licznik pomiarów z EEPROM
    unsigned int storedPomiar = EEPROM.read(EEPROM_POMIAR_ADDR) | (EEPROM.read(EEPROM_POMIAR_ADDR + 1) << 8);
    if (storedPomiar > 0) {
      licznikPomiarow = storedPomiar;
    } else {
      // Jeśli EEPROM pusty, spróbuj odczytać z pliku
...
void loop() {
  ...
  if (SD.begin(chipSelect)) {
    ...
    if (plik) {
      ...
      // --- Zapisz licznik pomiarów do EEPROM po KAŻDYM pomiarze ---
      EEPROM.write(EEPROM_POMIAR_ADDR, licznikPomiarow & 0xFF);
      EEPROM.write(EEPROM_POMIAR_ADDR + 1, (licznikPomiarow >> 8) & 0xFF);
    }
    ...
    if (pomiaryBezKarty >= 3) {
      ...
      // --- Zapisz licznik pomiarów do EEPROM przed resetem ---
      EEPROM.write(EEPROM_POMIAR_ADDR, licznikPomiarow & 0xFF);
      EEPROM.write(EEPROM_POMIAR_ADDR + 1, (licznikPomiarow >> 8) & 0xFF);
